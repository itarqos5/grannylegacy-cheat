...
namespace
{
    ...
    using GUIBoxFn = void (*)(UnityRect, void*, void*);
    using GUILabelFn = void (*)(UnityRect, void*, void*);
    using GUIButtonFn = bool (*)(UnityRect, void*, void*);
    using GUISetBackgroundColorInjectedFn = void (*)(UnityColor*);
    ...
    GUISetBackgroundColorInjectedFn g_setBackgroundColorInjected = nullptr;
    ...
    void EnsureUnityGuiFunctions()
    {
        ...
        ResolveIcall(g_setBackgroundColorInjected, "UnityEngine.GUI::set_backgroundColor_Injected(UnityEngine.Color&)");
        ...
        // Direct RVA fallback for this game build when icall/exports are unavailable.
        if (!g_guiBox)
        {
            g_guiBox = reinterpret_cast<GUIBoxFn>(reinterpret_cast<std::uintptr_t>(gameAssembly) + rva::GuiBoxRectString);
            logger::Warn("Using RVA fallback for GUI::Box");
        }
        if (!g_guiLabel)
        {
            g_guiLabel = reinterpret_cast<GUILabelFn>(reinterpret_cast<std::uintptr_t>(gameAssembly) + rva::GuiLabelRectString);
            logger::Warn("Using RVA fallback for GUI::Label");
        }
        if (!g_setBackgroundColorInjected)
        {
            g_setBackgroundColorInjected = reinterpret_cast<GUISetBackgroundColorInjectedFn>(reinterpret_cast<std::uintptr_t>(gameAssembly) + rva::GuiSetBackgroundColorInjected);
            logger::Warn("Using RVA fallback for GUI::set_backgroundColor_Injected");
        }
        ...
        if (!g_loggedGuiResolve)
        {
            logger::Info(std::string("GUI resolve status box=") + (g_guiBox ? "yes" : "no") +
                ...
                " bgColor=" + (g_setBackgroundColorInjected ? "yes" : "no"));
    ...
    void DrawExpandedMenu()
    {
        ...
        if (g_setBackgroundColorInjected)
        {
            UnityColor bg{0.08f, 0.30f, 0.85f, 0.92f};
            g_setBackgroundColorInjected(&bg);
        }
        ...
        g_guiBox(panel, NewUnityString(""), nullptr);
        g_guiLabel(title, NewUnityString("NEXUS MODLOADER"), nullptr);

        if (g_guiButton && g_guiButton(minButton, NewUnityString("-"), nullptr))
        {
            g_minimized = true;
        ...
        {
            g_guiLabel(minButton, NewUnityString("-"), nullptr);
        }

        g_guiLabel(freezeLabel, NewUnityString("Freeze Granny"), nullptr);
        if (g_guiButton && g_guiButton(freezeToggle, NewUnityString(feature::freeze::IsEnabled() ? "ON" : "OFF"), nullptr))
        {
        ...
        {
            g_guiLabel(freezeToggle, NewUnityString(feature::freeze::IsEnabled() ? "ON" : "OFF"), nullptr);
        }
    }
    ...
    void DrawMinimizedMenu()
    {
        ...
        g_guiBox(bar, NewUnityString(""), nullptr);
        g_guiLabel(title, NewUnityString("NEXUS MODLOADER"), nullptr);

        if (g_guiButton && g_guiButton(plusButton, NewUnityString("+"), nullptr))
        {
            g_minimized = false;
        ...
        {
            g_guiLabel(plusButton, NewUnityString("+"), nullptr);
        }
    }
